...
def import_bd(registration_username, registration_password):
    ...
    c = access
    query1 = (
        f" INSERT INTO registration (login,password,access) VALUES('{a}','{b}','{c}' )"
    )
    cur.execute(query1)
    con.commit()
    cur.close()
...
def import_redactor_bd(redactor_username, redactor_password, redactor_access):
    ...
    c = redactor_access
    query1 = (
        f" INSERT INTO admin_user (login,password,access) VALUES('{a}','{b}','{c}' )"
    )
    cur.execute(query1)
    con.commit()
    cur.close()
...
def check_user_bd(entrance_password):
    """
    Проверка на наличие договора у пользователя :
    """
    con = sqlite3.connect("ttk.db")
    cur = con.cursor()
...
def check_admin_bd(entrance_username, entrance_password):
    ...
    if result:
        if result[3] == 1:  # Проверяем значение столбца access (индекс 2 в кортеже)
            return True  # Возвращаем , если access равен 1
        else:
